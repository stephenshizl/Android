...
sp<CameraHardwareInterface> mCamHardware;

camera_module_t * FT_mModule = NULL ;
...
extern "C" int FTCamera_TakePicture(void)
{
    ...
    mCamHardware->enableMsgType(CAMERA_MSG_SHUTTER|CAMERA_MSG_POSTVIEW_FRAME|CAMERA_MSG_RAW_IMAGE|CAMERA_MSG_COMPRESSED_IMAGE);
...
// shao.wenqi@byd.com; add ; add start autofocus function
...
//szy    ioctl(fd, FBLOCKFB, &vi);
    ...
    mCamHardware->setCallbacks(notifyCallback, dataCallback, dataCallbackTimestamp, 
                                                                    (void *)cameraId);
...
extern "C" int FTCamera_StopPreview(void)
{
    ...
    mCamHardware->disableMsgType(CAMERA_MSG_SHUTTER | CAMERA_MSG_POSTVIEW_FRAME | CAMERA_MSG_RAW_IMAGE | CAMERA_MSG_COMPRESSED_IMAGE);
