...
static int get_framebuffer(GGLSurface *fb)
{
    ...
    for(i = 0; i < FB_NUM; i++){
        ...
        fb->height = vi.yres;
        fb->stride = fi.line_length/PIXEL_SIZE;
        fb->data = (void*) (((unsigned) bits) + i * (vi.yres * fb->stride * vi.bits_per_pixel / 8));
        fb->format = PIXEL_FORMAT;
        memset(fb->data, 0, vi.yres * fb->stride * PIXEL_SIZE);
        fb++;
    }
...
void draw_grid(int w, int h, uint16_t* loc) {
  int i, j;
  int v;
  int stride = gr_framebuffer->stride;//fi.line_length / (vi.bits_per_pixel >> 3);
...
int ft_display_test_lte(int type)
{
 ...
 system("input tap 334 443");
    break;
  case S_RED:
 system("am start -n com.app.Diagapp/com.app.Diagapp.DiagappLcd");
    break;
  case S_GREEN:
