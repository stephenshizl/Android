...
public class Framework extends ListActivity {
	...
	TextView nvTextView;
        ...
        private static final char FLAG_PAS ='1';
        private static final char FLAG_FAL = '0';
        //Begin yu.hongxia@byd.com 20150111, add for set NV to zero in first time.
        private static final int CLEAR_FLAG = 1;
        private int resetNV = 0;
        private static final int TOTAL_ITEM = 26;
        //end
	static {
		...
		if (ActivityManager.isUserAMonkey())
			...
			setListAdapter(mBaseAdapter);
                    ...
                    if (mQcNvItems == null){
                        ...
                        myHandler.sendEmptyMessageDelayed(CLEAR_FLAG, 100);
        }
        //Begin yu.hongxia@byd.com 201501111, add for set NV to zero in first time.
        Handler myHandler = new Handler() {
            public void handleMessage(Message msg) {
                switch (msg.what) {
                    case CLEAR_FLAG:
                        for (resetNV = 0; resetNV < TOTAL_ITEM; resetNV++) {
                      /*  try {
                          //yuyao  mQcNvItems.setCITResult((char)resetNV, (char)0);
                        }catch (IOException e) {
                            loge("Cannot write NV");
                        }*/
                        } 
                    default:
                        break;
	...
	protected void onActivityResult(int requestCode, int resultCode, Intent data) {
		...
		//Begin yu.hongxia@byd.com add for NV flag
		if(resultCode == RESULT_OK) {
		    logd("resultCode is OK,write "+FLAG_PAS+" to NV,item= "+ requestCode);
		  /*  try {
		     //yuyao   mQcNvItems.setCITResult((char) requestCode ,FLAG_PAS);
		        loge("wait write NV");
		    } catch (IOException e) {
		        logd("Cannot read NV Settings");
		    }*/
		}else {
		    logd("resultCode is CANCLE,write "+FLAG_FAL+" to NV,item= "+ requestCode);
		   /* try {
		     //yuyao    mQcNvItems.setCITResult((char) requestCode,FLAG_FAL);
		    } catch (IOException e) {
		        loge("Cannot read NV Settings");
		    }*/
		}
		//end
			mBaseAdapter.notifyDataSetChanged();
	...
	private BaseAdapter mBaseAdapter = new BaseAdapter() {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
            ...
            text.setText(getString(getResources().getIdentifier(((String) (MainApp.getInstance().mItemList.get(position).get("title"))), "string", getPackageName())));
