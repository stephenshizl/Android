...
void YUV2RGB565(unsigned char *src, __u16 *dst_ori, int width, int height)
{
    unsigned char *src0;
    unsigned char *src1;
    unsigned char *src2;
...
int ftscreen_fb_open()
{
    ...
    //yhx gr_framebuffer[1].data = (GGLubyte*)(((unsigned) bits) + (vinfo.yres * vinfo.xres * vinfo.bits_per_pixel / 8));
    ...
    //yhxgr_framebuffer[2].data = (GGLubyte*)(((unsigned) bits) + 2 * (vinfo.yres * vinfo.xres * vinfo.bits_per_pixel / 8));
...
int ftscreen_write(unsigned char *yuv_buffer)
{
        ...
        if (frameCnt >= 0 && frameCnt <= 2)
        {
            ...
            {
                ALOGV("dumping data");
                written = write(file_fd, (uint8_t*)yuv_buffer, 320 *480 * 3 / 2);
                if (written < 0)
                {
