...
static int allocPMEM(struct pmemDev *thisPMEM, unsigned int size)
{
        ...
        BYD_FTPMEM_TRACE(" %s() : L( %d ) , opened /dev/ion  devfd : %d , mapfd : %d , size : %d", __func__, __LINE__, thisPMEM->ion_device_fd, thisPMEM->ion_alloc_fd.fd, size);
        ...
        BYD_FTPMEM_TRACE(" %s() : L( %d ) ,phys lookup success virt=0x%x -- FD=%d -- size - %d", __func__, __LINE__, thisPMEM->pmem_buf, thisPMEM->pmem_fd, thisPMEM->pmem_size);
    #else
        if ((unsigned int)thisPMEM->pmem_size >= size && thisPMEM->pmem_buf)
        {
            memset(thisPMEM->pmem_buf, 0x00, thisPMEM->pmem_size);
            BYD_FTPMEM_TRACE(" %s() : L( %d ) ,PMEM reuse successful (%d bytes at %p)\n", __func__, __LINE__, thisPMEM->pmem_size, thisPMEM->pmem_buf);
            return 0;
        }
        // First Allocate PMEM
