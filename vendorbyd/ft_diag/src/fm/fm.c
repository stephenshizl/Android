...
#include <stdarg.h>
    ...
    #define V4L2_AUDIO_MUTE ((0x980000 | 0x900) + 9)
                             //control id,refrence fmRxControl.java
    int fm_fd =  - 1;
    int fm_open()
    {
    ...
    int fm_set_frequent(char *freq)
    {
        ...
        if (*freq != '\0' && *(freq + 1) != '\0'
        frequency = (*freq - '0') *10+*(freq + 1) - '0'; freq_struct.frequency = (frequency *TUNE_MULT / 1000);
