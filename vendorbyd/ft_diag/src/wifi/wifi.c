...
int broadcomm_wifi_pull_file(char *phone_file_path, char *pc_file_path,
                             unsigned char *data, int data_len, int file_id,
                             char *file_stat);
int broadcomm_wifi_push_file(char *pc_file_path, char *phone_file_path,
                             unsigned char *data, int data_num, int data_len,
                             char del_flag);
...
/*Modify by yu.hongxia@byd.com for wifi init test , 20150117*/
pthread_t ntid;
...
static void stop_ptt_socket_app1() {
    char str[64] = { 0 };
...
int exec_wifi_init()
{
    //stop_ptt_socket_app1();
    return system("busybox killall ptt_socket_app; sleep 1; /system/bin/ptt_socket_app -v -d -f&");
}
int atheros_wifi_init(void)
{
    int ret = -1;
    ...
    //ret = system("ptt_socket_app -v -d -f");
    //system("busybox killall ptt_socket_app");
    ret = pthread_create(&ntid, NULL, exec_wifi_init, NULL);
    if(ret != 0)
    {
...
int atheros_wifi_close(void)
{
 ...
 //   system("rmmod wlan");
    return system("busybox killall ptt_socket_app");
    return 0;
}
int atheros_wifi_scan_ssid(char *response_ssid_list, int list_length)
{
...
int atheros_wifi_tx_set(int mode, int channel, int rate, int power_level, int sine_wave)
{
    ...
    memset(cmd, 0x00, 100);
    if (WIFI_DIAG_802_11_A_MODE == mode || WIFI_DIAG_802_11_G_MODE == mode)
    {
...
int atheros_wifi_rx_report(char *response_packet, int pkt_length)
{
    FILE *fp;
    FT_LOG("[%s,%d]DIAG ATHEROS WIFI\n", __func__, __LINE__);
    system("iwpriv wlan0 get_rxpktcnt > /data/misc/wifi/rxpktcnt");
           //get rx packet count
